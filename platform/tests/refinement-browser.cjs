...
(async()=>{const browser=await chromium.launch();try{
 ...
 await page.goto(base+'/privacy');await page.getByRole('heading',{name:'Privātuma politika',exact:true}).waitFor();await page.getByRole('heading',{name:'Kas ir atbildīgs',exact:true}).waitFor();assert(!(await page.locator('main').innerText()).includes('The event organizer decides'));
 await page.goto(base+'/sample-workspace');await page.getByText('Mani pasākumi',{exact:true}).first().waitFor();assert(!(await page.locator('.workspace').innerText()).includes('Organizer workspace'));await page.screenshot({path:'platform/test-results/workspace-lv.png',fullPage:true});
