...
(async()=>{
 ...
 try{
  ...
  await page.getByLabel('Confirm password',{exact:true}).fill('Browser-new-password-123!');await page.getByRole('button',{name:'Save password'}).click();await page.waitForURL('**/login');password='Browser-new-password-123!';
