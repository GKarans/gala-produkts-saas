...
async function render(){const request=++version;disposeGuest();disposeWorkspace();const app=document.getElementById('app'),path=location.pathname;document.getElementById('dialog').close();document.getElementById('dialog').onkeydown=null;try{
 ...
 else if(path.startsWith('/auth/')){const reset=path==='/auth/reset',emailChange=path==='/auth/email',purpose=path.split('/')[2],query=new URLSearchParams(location.search),fragment=new URLSearchParams(location.hash.slice(1)),token=query.get('token')||query.get('token_hash'),accessType=reset?'recovery':'email_change',accessToken=(reset||emailChange)&&!token&&fragment.get('type')===accessType?fragment.get('access_token'):null;if(token||accessToken)history.replaceState({},'',path);html=nav()+`<main id="main" class="narrow"><h1>${reset?'Set a new password':'Confirm your email'}</h1><form id="consume">${reset?field('New password','password','','password','required minlength="12" maxlength="128" autocomplete="new-password"')+field('Confirm password','password_confirm','','password','required minlength="12" maxlength="128" autocomplete="new-password"'):'<p>Complete your email verification to continue.</p>'}<p class="error" role="alert"></p><button type="submit" class="primary">${reset?'Save password':'Verify email'}</button></form></main>`+footer();wire=()=>wireForm(document.getElementById('consume'),async input=>{if(reset&&input.password!==input.password_confirm)throw new Error(t('Passwords do not match.'));const r=await api('/auth/consume',{method:'POST',body:{...(token?{token}:{}),...(accessToken?{access_token:accessToken,type:accessType}:{}),purpose,...input}});toast(r.message);history.replaceState({},'', '/login');render();});}
...
document.addEventListener('click',e=>{const menu=document.querySelector('.mobile-workspace-nav');if(menu?.open&&!menu.contains(e.target))menu.open=false;});
document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;const menu=document.querySelector('.mobile-workspace-nav');if(menu?.open){menu.open=false;menu.querySelector('summary').focus();}});
