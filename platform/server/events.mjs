...
import {PLANS,eventState,folder} from '../shared/plans.js';
...
export function eventService(db,mail) {
 ...
 const own=async(user,id,tx=db)=>{const event=(await tx.query("select * from events where id=$1 and owner_id=$2 and status<>'deleted'"+(tx===db?'':' for update'),[id,user.id])).rows[0];requireThat(event,404,'Event not found.');return event;};
 ...
 return {
  ...
  async list(user){return(await db.query("select e.*,count(m.id) filter(where m.status='uploaded')::int as photo_count,coalesce(sum(m.bytes+m.thumbnail_bytes) filter(where m.status in ('uploaded','pending')),0)::bigint as bytes from events e left join media m on m.event_id=e.id where e.owner_id=$1 and e.status<>'deleted' group by e.id order by e.created_at desc",[user.id])).rows.map(e=>({...e,state:eventState(e)}));},
  async save(user,input,id=null){return db.transaction(async tx=>{await tx.query('select id from accounts where id=$1 for update',[user.id]);const existing=id?await own(user,id,tx):null;const sub=(await tx.query('select * from subscriptions where account_id=$1',[user.id])).rows[0];const publication=existing?(await tx.query('select * from event_publications where event_id=$1',[id])).rows[0]:null;const plan=publication?.entitlement||PLANS.studio;const times=schedule(input);requireThat(Date.parse(times.ends_at)-Date.parse(times.starts_at)<=plan.durationDays*86400000,400,`This plan supports events up to ${plan.durationDays} days.`);const name=text(input.name,80);const preset=COVERS.find(c=>c.id===input.coverPreset);const appearance={title:String(input.title||name).slice(0,80),subtitle:String(input.subtitle||'').slice(0,180),button:String(input.button||existing?.appearance.button||'Take a photo').slice(0,28),chooseButton:String(input.chooseButton||existing?.appearance.chooseButton||'Choose photos').slice(0,28),cover:preset?.url||existing?.appearance.cover||(COVERS.some(c=>c.url===input.cover)?input.cover:'/assets/garden-gathering.webp'),...normalizeCover({...existing?.appearance,...input})};
   if(existing){if(publication){requireThat(Date.parse(times.ends_at)<=Date.parse(publication.schedule_deadline),409,'This event cannot be moved beyond its scheduling window.');const captured=(await tx.query("select id from media where event_id=$1 limit 1",[id])).rows.length;if(captured)requireThat(Date.parse(times.starts_at)===Date.parse(existing.starts_at)&&Date.parse(times.ends_at)===Date.parse(existing.ends_at),409,'Event times cannot change after photos have been submitted. Pause uploads if needed.');}requireThat(eventState(existing)!=='completed'&&existing.status!=='archived',409,'A completed or archived event cannot be rescheduled.');if(existing.appearance.cover_key){if(preset)await tx.query("insert into jobs(id,owner_id,event_id,type,payload) values($1,$2,$3,'object-cleanup',$4)",[uuid(),user.id,id,JSON.stringify({keys:[existing.appearance.cover_key]})]);else appearance.cover_key=existing.appearance.cover_key;}await tx.query('update events set name=$1,description=$2,starts_at=$3,ends_at=$4,time_zone=$5,appearance=$6,retention_at=$7 where id=$8',[name,String(input.description||'').slice(0,500),times.starts_at,times.ends_at,times.time_zone,JSON.stringify(appearance),new Date(Date.parse(times.ends_at)+existing.entitlement.retentionDays*86400000).toISOString(),id]);}
  ...
  async action(user,id,input){return db.transaction(async tx=>{await tx.query('select id from accounts where id=$1 for update',[user.id]);const e=await own(user,id,tx);const action=input.action;
   if(action==='publish'){
    ...
    requireThat(Date.parse(e.ends_at)-Date.parse(e.starts_at)<=plan.durationDays*86400000,409,`This allowance supports events up to ${plan.durationDays} days.`);
   ...
   else if(action==='share'){requireThat(eventState(e)==='completed',409,'Sharing becomes available when the event ends.');const days=Number(input.days);requireThat(Number.isInteger(days)&&days>=1&&days<=e.entitlement.shareDays,400,`Choose 1 to ${e.entitlement.shareDays} days.`);await tx.query('update events set share_enabled=$1,share_expires=least(now()+($2*interval \'1 day\'),retention_at) where id=$3',[input.enabled===true,days,id]);}
  ...
  async guest(slug){const e=(await db.query("select * from events where slug=$1 and status='published' and retention_at>now()",[slug])).rows[0];requireThat(e,404,'Event not found. Check the link and try again.');return e;},
  async join(e,input){requireThat(eventState(e)==='live',409,'Photo upload is not available for this event right now.');const id=uuid(),secret=token(),name=text(input.name,80);await db.query('insert into guests(id,event_id,name,token_hash,storage_prefix) values($1,$2,$3,$4,$5)',[id,e.id,name,hash(secret),`${e.storage_prefix}/guests/${folder(name,id)}`]);return {id,name,token:secret};},
