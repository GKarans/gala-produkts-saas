...
export function eventService(db,mail) {
 ...
 return {
  ...
  async save(user,input,id=null){return db.transaction(async tx=>{
   ...
   const existing=id?await own(user,id,tx):null;
  ...
  async join(e,input){requireThat(eventState(e)==='live',409,'Photo upload is not available for this event right now.');const id=uuid(),secret=token(),name=text(input.name,80);await db.query('insert into guests(id,event_id,name,token_hash,storage_prefix) values($1,$2,$3,$4,$5)',[id,e.id,name,hash(secret),`${e.storage_prefix}/${folder(name,id)}`]);return {id,name,token:secret};},
