...
export function supabaseAuthService(db,{origin,mail,fetcher=fetch}){
 ...
 const sync=async user=>{requireThat(user?.id&&user.email_confirmed_at,403,'Verify your email before signing in.');const metadata=user.user_metadata||{},preferences={service:true,marketing:false,locale:metadata.locale==='lv'?'lv':'en'},profile=metadata.profile&&typeof metadata.profile==='object'&&!Array.isArray(metadata.profile)?metadata.profile:{};await db.query('insert into accounts(id,email,name,verified,preferences,profile) values($1,$2,$3,true,$4,$5) on conflict(id) do update set email=excluded.email,verified=true',[user.id,user.email,String(metadata.name||metadata.full_name||'Organizer').slice(0,80),JSON.stringify(preferences),JSON.stringify(profile)]);await db.query('insert into subscriptions(account_id) values($1) on conflict do nothing',[user.id]);const account=(await db.query('select * from accounts where id=$1 and deleted_at is null',[user.id])).rows[0];requireThat(account,403,'This account is no longer available. Contact support.');return account;};
 ...
 return{mail,
  ...
  async consume(input){
   if(input.access_token){requireThat(input.purpose==='verify'&&input.type==='signup'&&typeof input.refresh_token==='string',400,'This confirmation link is invalid. Request a new one.');const providerSession={access_token:text(input.access_token,8192),refresh_token:text(input.refresh_token,8192),expires_at:Math.floor(Date.now()/1000)+Math.min(86400,Math.max(60,Number(input.expires_in)||3600))};const user=await request('user',null,providerSession.access_token);const account=await sync(user),sessionToken=token();await db.query('insert into sessions(token_hash,account_id,expires_at,provider_session) values($1,$2,now()+interval \'7 days\',$3)',[hash(sessionToken),account.id,seal(providerSession)]);return{message:'Email confirmed. Your account is ready.',cookie:`lumiq_session=${sessionToken}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=604800`};
   }
