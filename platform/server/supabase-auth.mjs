...
export function supabaseAuthService(db,{origin,mail,fetcher=fetch}){
 ...
 return{mail,
  ...
  async update(user,input){const session=await request('token?grant_type=password',{email:user.email,password:input.password});requireThat(session.user.id===user.id,403,'Account mismatch.');await request('user',{data:{name:text(input.name,80)},...(input.email!==user.email?{email:email(input.email)}:{})},session.access_token,'PUT');await db.query('update accounts set name=$1,preferences=$2 where id=$3',[text(input.name,80),{service:true,marketing:input.marketing===true,locale:input.locale==='lv'?'lv':'en'},user.id]);return{message:'Profile saved. Check the confirmation email if you changed your address.'};}
 };
}
