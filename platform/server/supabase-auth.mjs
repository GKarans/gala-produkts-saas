...
export function supabaseAuthService(db,{origin,mail,fetcher=fetch}){
 ...
 const sync=async user=>{requireThat(user?.id&&user.email_confirmed_at,403,'Verify your email before signing in.');const preferences={service:true,marketing:false,locale:user.user_metadata?.locale==='lv'?'lv':'en'};await db.query('insert into accounts(id,email,name,verified,preferences) values($1,$2,$3,true,$4) on conflict(id) do update set email=excluded.email,verified=true',[user.id,user.email,String(user.user_metadata?.name||user.user_metadata?.full_name||'Organizer').slice(0,80),JSON.stringify(preferences)]);await db.query('insert into subscriptions(account_id) values($1) on conflict do nothing',[user.id]);const account=(await db.query('select * from accounts where id=$1 and deleted_at is null',[user.id])).rows[0];requireThat(account,403,'This account is no longer available. Contact support.');return account;};
 ...
 return{mail,
  ...
  async register(input){requireThat(typeof input.password==='string'&&input.password.length>=12&&input.password.length<=128,400,'Use a password with 12 to 128 characters.');await request(`signup?redirect_to=${encodeURIComponent(origin+'/auth/verify')}`,{email:email(input.email),password:input.password,data:{name:text(input.name,80),locale:input.locale==='lv'?'lv':'en'}});return{message:'Check your email to verify your account.'};},
