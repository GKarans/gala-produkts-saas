...
export function jobService(db,files,mail){
 ...
 const exportJob=async job=>{
  ...
  const flush=async()=>{
   ...
   await db.query('update jobs set result=$1 where id=$2',[{parts,partial:true},job.id]);
   batch=[];bytes=0;
  };
 ...
 const cleanup=async job=>{
  ...
  const isEvent=['cleanup','retention-cleanup'].includes(job.type);const rows=(await db.query(isEvent?'select * from media where event_id=$1':"select * from media where event_id=$1 and id=any($2::uuid[]) and status='deleted'",isEvent?[job.event_id]:[job.event_id,job.payload.ids])).rows;for(const m of rows){await files.remove(m.object_key);await files.remove(m.thumbnail_key);}if(isEvent){const event=(await db.query('select appearance from events where id=$1',[job.event_id])).rows[0];if(event?.appearance.cover_key)await files.remove(event.appearance.cover_key);if(event?.appearance.qr_background_key)await files.remove(event.appearance.qr_background_key);const exports=(await db.query("select result from jobs where event_id=$1 and type='export'",[job.event_id])).rows;for(const e of exports)for(const part of e.result.parts||[])await files.remove(part.key);await db.query("update media set status='deleted',deleted_at=coalesce(deleted_at,now()) where event_id=$1",[job.event_id]);await db.query("update events set status=$2,paused=true,share_enabled=false,gallery_cover_id=null where id=$1",[job.event_id,job.type==='cleanup'?'deleted':'archived']);}return{removed:rows.length};};
 return{
  queue,
  ...
  async requestExport(user,event,input){requireThat(Date.parse(event.retention_at)>Date.now(),410,'Photo retention has ended. This event is archived and its photos are no longer available.');requireThat(Date.now()>=Date.parse(event.ends_at),409,'Exports become available after the event.');const ids=input.ids;if(ids)requireThat(Array.isArray(ids)&&ids.length>0&&ids.length<=5000,400,'Choose photos to export.');const rows=(await db.query("select id from media where event_id=$1 and status='uploaded'"+(ids?' and id=any($2::uuid[])':''),ids?[event.id,ids]:[event.id])).rows;requireThat(rows.length&&(!ids||rows.length===new Set(ids).size),400,'No photos or some selected photos are unavailable.');const snapshot=rows.map(x=>x.id).sort();const previous=(await db.query("select id from jobs where owner_id=$1 and event_id=$2 and type='export' and status in ('queued','processing','ready') and payload=$3 and created_at>now()-interval '6 days' order by created_at desc limit 1",[user.id,event.id,{ids:snapshot}])).rows[0];return previous||queue(user.id,event.id,'export',{ids:snapshot});},
