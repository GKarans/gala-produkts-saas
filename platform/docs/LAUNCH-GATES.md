...
- [x] Closed-test database foundation only: migrations `001-platform` through `007-jsonb-parameter-encoding`, RLS on all 17 protected tables, anon/authenticated SELECT denied, and restricted `lumiq_runtime` role provisioned and privilege-checked. Evidence: owner ran `migrate-new-test.ps1` and `provision-test-db-role.ps1` against the separate Free test project on 2026-09-23/24. This is not production evidence.
...
- [x] Closed-test billing page works with payment controls disabled: after version `b1b7fe9e-78a4-4e7a-a422-4a506d63608f`, the owner-authenticated page showed Explore `1 / 1` and displayed that payments are disconnected, with no JavaScript page error. This does not imply a paid plan or prove billing provider integration.
