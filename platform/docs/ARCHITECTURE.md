...
The migration runner applies `001-platform` from `platform/server/schema.sql` and numbered migrations through `012-tier-photo-capacity` from `platform/server/migrations/`, using the single server-only manifest shared by local DB startup, the guarded migrator and Worker startup verification. A locked, checksummed ledger rejects changed applied SQL outside local development. API startup refuses missing versions. Further changes require forward entries and a backup/restore procedure. This does not migrate MVP data; real target PostgreSQL migration/restore acceptance remains pending.
