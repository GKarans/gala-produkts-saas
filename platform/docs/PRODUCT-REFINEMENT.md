...
QR print tools include four original Lumiq table-card designs, a standalone square QR PNG for external layouts, and localized poster text. The per-event editor saves the template, one of ten bundled Google Fonts, title size, QR size, and bounded title/QR positions; both blocks can be dragged directly in the preview or adjusted with a keyboard. Native design text is placed directly over the artwork without a white text panel, while the QR keeps a light quiet zone for reliable scanning. Printing opens only the finished design image, never the editor interface. Organizers can download the standalone QR, complete an independent design in Canva, and upload the exported JPEG, PNG or WebP as the final design. Lumiq does not add text or a QR code to that uploaded file. Customer uploads remain private to the event and are never promoted to a shared template automatically.
